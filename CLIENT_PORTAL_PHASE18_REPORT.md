...
**Result: 1965 / 1965 checks = 100.0 %** — UI 558, backend 1378, live 29.
(Includes the follow-up in §6: a generated link is a complete http address, produced by
the server itself.)
...
UI harness 558/558 (34 new checks: helper units, both rendered reveal states, the
section chip's copied address, the panel spending the server's address, no credential in
any URL), backend 1378/1378 (17 new: the address contract, the server generating the
address from the request's own site, a hostile origin never echoed, redemption lands on
the destination, a passkey address opens nothing, tampering, and that the list can never
re-show an issued address), live 29/29 (the server-generated address is fetched on the
running site, redeemed, and lands on the destination project; the shipped bundle carries
both the section link and the link parser).

### The address is generated by the server, for the site in use
