  ...
  return <main className="min-h-screen bg-[#f7faf8] px-4 py-7 sm:px-6 sm:py-12 print:bg-white print:p-0">
    <article className="mx-auto max-w-4xl overflow-hidden rounded-3xl border border-emerald-100 bg-white shadow-xl shadow-emerald-950/5 print:max-w-none print:rounded-none print:border-0 print:shadow-none">
      <header className="border-b border-emerald-100 bg-gradient-to-br from-[#f7fffa] via-[#ecf9f0] to-[#def4e6] p-6 text-slate-800 sm:p-10 print:bg-white">
        <div className="flex items-start justify-between gap-5"><div className="flex min-w-0 items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-emerald-200 bg-white text-emerald-700 shadow-sm"><FileText className="h-5 w-5" /></div><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-700">Code Rx Vault · Shared document</p><h1 className="mt-2 text-2xl font-black tracking-tight text-slate-800 sm:text-3xl">{document?.title || 'Opening secure document…'}</h1>{document?.documentCode && <p className="mt-2 text-xs font-black uppercase tracking-wider text-emerald-700">{document.documentCode}</p>}</div></div><div className="flex shrink-0 flex-wrap justify-end gap-2 print:hidden">{document?.canDownload && <><button onClick={download} className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-50"><Download className="h-4 w-4" />Download</button><button onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50"><Printer className="h-4 w-4" />Print</button></>}<button onClick={() => { window.location.hash = ''; onClose(); }} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50"><ArrowLeft className="h-4 w-4" />Close</button></div></div>
        {document && !document.canDownload && <div role="status" className="mt-5 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900 print:hidden"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" /><span>{document.downloadMessage || 'This document is shared for reading only. Download and print are unavailable for this link.'}</span></div>}
