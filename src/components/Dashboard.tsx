...
const Overview = ({ firstName, member, leaderboard, vaultHome, notifications, memberError, loading, onCodenameClaimed, onOpenVault }: { firstName: string; member: any; leaderboard: any[]; vaultHome: any; notifications: any; memberError: string; loading: boolean; onCodenameClaimed: () => void; onOpenVault: () => void }) => {
  ...
  return <div className="space-y-8"><header className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-3xl font-black text-slate-800">Welcome, {firstName} 👋</h2><p className="text-slate-500">Your real Code Rx member space is ready for learning, building, and contributing.</p></div><div className="flex items-center gap-3"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Search resources..." className="rounded-full border border-slate-200 bg-white py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#fff1ae]" /></div><NotificationCenter /></div></header>{memberError && <p className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-600">{memberError}</p>}{member && !member.codename && <CodenameBallot codenamePath={member.codenamePath} onClaimed={onCodenameClaimed} />}<div className="grid grid-cols-2 gap-4 lg:grid-cols-4">{[{ label: 'Calcitonins', value: `${Number(member?.points || 0).toLocaleString()} CAL`, icon: Zap, color: 'text-amber-700', bg: 'bg-amber-100' }, { label: 'Accessible Docs', value: loading ? '—' : accessibleDocuments.toLocaleString(), icon: BookOpen, color: 'text-sky-700', bg: 'bg-sky-100' }, { label: 'Projects', value: loading ? '—' : String((vaultHome?.sections || []).find((section: any) => section.slug === 'projects')?.documentCount || 0), icon: FolderKanban, color: 'text-violet-700', bg: 'bg-violet-100' }, { label: 'Updates', value: Number(notifications?.unreadCount || 0).toLocaleString(), icon: MessageSquare, color: 'text-emerald-700', bg: 'bg-emerald-100' }].map((stat) => <div key={stat.label} className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"><div className={`${stat.bg} ${stat.color} mb-4 grid h-10 w-10 place-items-center rounded-lg`}><stat.icon className="h-6 w-6" /></div><p className="text-xs font-bold uppercase tracking-widest text-slate-400">{stat.label}</p><p className="text-2xl font-black text-slate-800">{stat.value}</p></div>)}</div><div className="grid gap-8 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2"><section className="rounded-3xl border border-emerald-100 bg-[#f2fbf5] p-7 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-700">Calcitonin balance</p><h3 className="mt-2 text-xl font-black text-slate-800">Your verified Calcitonin balance</h3><p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">PHANTOM-approved adjustments and verified achievements update your Calcitonins. Every change is recorded in your notification inbox.</p></div><span className="rounded-2xl border border-emerald-100 bg-white px-4 py-3 text-2xl font-black text-emerald-700 shadow-sm">{Number(member?.points || 0).toLocaleString()} CAL</span></div></section><section className="rounded-3xl border border-slate-100 bg-white p-8 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="text-xl font-black text-slate-800">Recent Vault work</h3><p className="mt-1 text-sm text-slate-500">Only real documents you can access appear here.</p></div><button onClick={onOpenVault} className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-black uppercase tracking-wider text-emerald-700 hover:bg-emerald-100">Open Vault</button></div><div className="mt-5 space-y-2"><RecentItems items={vaultHome?.recentDocuments || []} label="documents" render={(document) => <div key={document.id} className="rounded-xl border border-slate-100 px-4 py-3"><p className="font-bold text-slate-800">{document.title}</p><p className="mt-1 text-xs text-slate-500">{document.document_code ? `${document.document_code} · ` : ''}{document.section_title || document.section_slug}</p></div>} />{!(vaultHome?.recentDocuments || []).length && <EmptyPanel icon={Archive} title="No Vault documents yet" text="Open the Vault when you are ready to review or contribute to Society documentation." action="Open Vault" onAction={onOpenVault} />}</div></section></div><Leaderboard leaderboard={leaderboard} /></div></div>;
};
