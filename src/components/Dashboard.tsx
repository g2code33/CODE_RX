...
const CommunityView = ({ notifications }: { notifications: any[] }) => <div className="space-y-6"><PortalHeader eyebrow="Community" title="Community updates" text="Real PHANTOM and delegated-member broadcasts are shown below." />{notifications.length ? <div className="space-y-3">{notifications.slice(0, 12).map((notification) => <article key={notification.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="font-black text-slate-800">{notification.title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{notification.message}</p><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-slate-400">{notification.sender_name || 'Code Rx Society'} · {notification.delivered_at || notification.created_at}</p></div>{notification.status === 'unread' && <span className="mt-1 h-2.5 w-2.5 rounded-full bg-emerald-500" />}</div></article>)}</div> : <PortalPage eyebrow="Community" title="No community updates yet" text="PHANTOM broadcasts and approved community notices will appear here when they are sent." icon={Users} action="Explore public community" onAction={() => browsePublic('community')} />}</div>;
