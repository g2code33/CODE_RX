          ...
          <div className="grid items-start gap-4 md:grid-cols-2">
            {content.sections.map((section) => (
              <motion.article key={section.id} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="brand-card brand-card-hover p-6 sm:p-8">
                ...
                <div className="whitespace-pre-line text-sm leading-7 text-[#a9bda3]">{section.content}</div>
              </motion.article>
            ))}
          </div>

          <div className="relative mt-16 overflow-hidden rounded-2xl border border-[#b8ff3d]/30 bg-[#0b1c10] p-8 text-center sm:p-12"><div className="brand-glow left-1/2 top-[-14rem] -translate-x-1/2 opacity-40" /><div className="relative z-10"><p className="brand-eyebrow justify-center">Official acceptance</p><h3 className="mt-5 text-2xl font-black text-[#f2f8ed] sm:text-3xl">Build with care. Build with purpose.</h3><p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-[#a9bda3]">By registering for Code Rx Society membership, you acknowledge that you have read, understood, and agreed to these Terms & Conditions.</p><p className="mt-7 text-sm font-black uppercase tracking-[0.12em] text-[#b8ff3d]">We don't just learn pharmacy. We build what moves it forward.</p><p className="mt-9 text-[0.64rem] font-black uppercase tracking-[0.18em] text-[#94a992]">Code Rx Society © 2026 / Ghana</p></div></div>
        </motion.div>
      </div>
    </section>
  );
