...
import {
  Bold, Check, ChevronLeft, ChevronRight, Code2, Command, Copy,
  FileText, Heading1, Heading2, History, ImagePlus, Italic, Link2,
  List, ListOrdered, LoaderCircle, Maximize2, Minimize2, Minus, MoreHorizontal,
  Paperclip, Plus, Quote, Redo2, Save, Search, Strikethrough, Table2,
  Underline, Undo2, X,
} from 'lucide-react';
import { db } from '../lib/cloudflare';
import {
  newBlock, parseDocumentContent, plainTextFromBlocks,
...
const sanitizeHtml = (value: string) => value
  ...
  .replace(/<(?!\/?(?:strong|b|em|i|u|s|strike|mark|a|code|br|span)\b)[^>]*>/gi, '');
...
export const VaultDocumentEditor = ({ document, section, projects, canEdit, canManage, onClose, onSaved }: VaultDocumentEditorProps) => {
  ...
  useEffect(() => {
    ...
    const key = draftStorageKey(document?.id || null, section?.slug || 'vault');
    try {
      const stored = localStorage.getItem(key);
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed?.draft?.title || parsed?.draft?.blocks?.length) {
          setDraft({ ...next, ...parsed.draft, blocks: parseDocumentContent({ blocks: parsed.draft.blocks }, '') });
          setSaveState('offline');
          setSaveMessage('Recovered local draft');
        } else setDraft(next);
      } else setDraft(next);
    } catch { setDraft(next); }
    setDocumentId(document?.id || null);
  ...
  const setBlocks = (next: VaultBlock[], remember = false) => {
    if (remember) {
      setUndo((history) => [...history, draft.blocks].slice(-60));
      setRedo([]);
    }
    setDraft((current) => ({ ...current, blocks: next }));
    setSaveState(navigator.onLine ? 'unsaved' : 'offline');
    setSaveMessage(navigator.onLine ? 'Unsaved changes' : 'Offline — draft protected locally');
  };
  ...
  const insertAfter = (afterId: string | null, type: VaultBlockType, patch: Partial<VaultBlock> = {}) => {
    ...
    const index = afterId ? draft.blocks.findIndex((item) => item.id === afterId) : draft.blocks.length - 1;
  ...
  const applyCommand = (command: typeof slashCommands[number]) => {
    ...
    if (currentId && command.type !== 'divider') {
      const current = draft.blocks.find((block) => block.id === currentId);
  ...
  const payload = () => ({
    ...
    contentJson: { version: 1, blocks: draft.blocks },
    content: plainTextFromBlocks(draft.blocks),
    tags: draft.tags,
    status: draft.status,
  ...
  const protectLocalDraft = () => {
    try { localStorage.setItem(draftStorageKey(documentId, section.slug), JSON.stringify({ savedAt: new Date().toISOString(), draft })); } catch { /* local storage unavailable */ }
  };
  ...
  const persist = async (manual = false) => {
    ...
    try {
      ...
      if (!currentId) {
        const result = await db.vault.createDocument({ section: section.slug, ...payload(), autosave: !manual });
        currentId = result.data.id;
        setDocumentId(currentId);
      } else {
        await db.vault.updateDocument(currentId, { ...payload(), autosave: !manual, changeNote: manual ? 'Saved from Code Rx Vault editor' : 'Autosaved document update' });
      }
      ...
      setSaveState('saved'); setSaveMessage(manual ? 'Saved just now' : 'Saved');
      onSaved(saved);
    } catch (error: any) {
      protectLocalDraft();
      setSaveState('error'); setSaveMessage(error?.message || "Couldn't save your changes. Retry when ready.");
    }
  };
  ...
  }, [draft.title, JSON.stringify(draft.blocks), JSON.stringify(draft.tags), draft.status, draft.relatedProjectId, saveState, canEdit]);
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'z' && !event.shiftKey && !editingText) {
        ...
        setUndo((history) => {
          ...
          setRedo((future) => [...future, draft.blocks]); setBlocks(previous, false);
  ...
  const uploadAttachment = async (file: File) => {
    setUploading(true);
    try {
      const result = await db.vault.uploadFile(file, section.slug, documentId || undefined);
  ...
  const shellClass = focusMode ? 'fixed inset-0 z-[130] overflow-y-auto bg-[#f8fafc] p-4 sm:p-8' : 'min-w-0';

  return <div className={shellClass}><div className="mx-auto max-w-[1180px]"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2 text-xs font-bold text-slate-500"><button onClick={focusMode ? () => setFocusMode(false) : onClose} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 hover:bg-slate-100"><ChevronLeft className="h-4 w-4" />{focusMode ? 'Exit focus' : `Vault / ${section.title}`}</button><span className="text-slate-300">/</span><span className="truncate text-slate-700">{draft.title || 'Untitled document'}</span></div><div className="flex flex-wrap items-center gap-2"><SaveIndicator state={saveState} message={saveMessage} /><button onClick={() => setFocusMode((value) => !value)} className="editor-icon-button" title="Focus mode">{focusMode ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button><button onClick={() => { void db.vault.documentVersions(documentId || 0).then(setVersions).then(() => setShowHistory(true)); }} disabled={!documentId} className="editor-icon-button" title="Version history"><History className="h-4 w-4" /></button>{canEdit && <button onClick={() => void persist(true)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white"><Save className="h-4 w-4" />Save</button>}</div></div>
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_210px]"><div className="min-w-0 rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4 sm:px-8"><input disabled={!canEdit} value={draft.title} onChange={(event) => { setDraft((current) => ({ ...current, title: event.target.value })); setSaveState('unsaved'); }} placeholder="Untitled document" className="w-full bg-transparent text-3xl font-black tracking-tight text-slate-900 outline-none placeholder:text-slate-300" /><div className="mt-4 flex flex-wrap items-center gap-2"><select disabled={!canEdit} value={draft.status} onChange={(event) => { setDraft((current) => ({ ...current, status: event.target.value as VaultDocumentDraft['status'] })); setSaveState('unsaved'); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-bold text-slate-600"><option value="draft">DRAFT</option><option value="in_review">IN REVIEW</option><option value="approved">APPROVED</option><option value="active">ACTIVE</option><option value="archived">ARCHIVED</option></select><select disabled={!canEdit} value={draft.visibility} onChange={(event) => { setDraft((current) => ({ ...current, visibility: event.target.value as VaultDocumentDraft['visibility'] })); setSaveState('unsaved'); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-bold text-slate-600"><option value="section">SECTION</option><option value="members">MEMBERS</option><option value="restricted">RESTRICTED</option></select><select disabled={!canEdit} value={draft.relatedProjectId || ''} onChange={(event) => { setDraft((current) => ({ ...current, relatedProjectId: event.target.value ? Number(event.target.value) : null })); setSaveState('unsaved'); }} className="max-w-52 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-bold text-slate-600"><option value="">No related project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select><TagInput tags={draft.tags} disabled={!canEdit} onChange={(tags) => { setDraft((current) => ({ ...current, tags })); setSaveState('unsaved'); }} /></div>{document && <p className="mt-3 text-[11px] font-medium text-slate-400">Author: {document.created_by_name || 'Code Rx member'} · Last modified by {document.updated_by_name || document.created_by_name || 'Code Rx member'} · Version history protected</p>}</div>
        {canEdit && <EditorToolbar onFormat={executeFormat} onInsert={(type: VaultBlockType, patch?: Partial<VaultBlock>) => insertAfter(activeBlockId, type, patch)} onPalette={() => setShowPalette(true)} onUndo={() => { const previous = undo[undo.length - 1]; if (previous) { setRedo((future) => [...future, draft.blocks]); setBlocks(previous, false); setUndo((history) => history.slice(0, -1)); } }} onRedo={() => { const next = redo[redo.length - 1]; if (next) { setUndo((history) => [...history, draft.blocks]); setBlocks(next, false); setRedo((future) => future.slice(0, -1)); } }} canUndo={Boolean(undo.length)} canRedo={Boolean(redo.length)} onUpload={() => fileInput.current?.click()} />}
        ...
        <div className="min-h-[60vh] px-5 py-7 sm:px-12 sm:py-10" onDragOver={(event) => { if (canEdit) event.preventDefault(); }} onDrop={(event) => { if (!canEdit) return; event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (file) void uploadAttachment(file); }}><div className="mx-auto max-w-3xl space-y-3">{showTemplates && canEdit && <TemplatePicker onChoose={applyTemplate} onClose={() => setShowTemplates(false)} />}{draft.blocks.map((block) => <DocumentBlockEditor key={block.id} block={block} canEdit={canEdit} active={activeBlockId === block.id} onFocus={() => setActiveBlockId(block.id)} onChange={(patch) => updateBlock(block.id, patch)} onSlash={(query) => setSlash({ blockId: block.id, query })} onInsert={(type) => insertAfter(block.id, type)} onRemove={() => removeBlock(block.id)} onMove={(direction) => moveBlock(block.id, direction)} onUpload={() => fileInput.current?.click()} />)}{canEdit && <button onClick={() => insertAfter(draft.blocks[draft.blocks.length - 1]?.id || null, 'paragraph')} className="mt-4 inline-flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-bold text-slate-400 hover:bg-slate-50 hover:text-emerald-600"><Plus className="h-4 w-4" />Add block</button>}</div></div>
        <div className="flex flex-wrap items-center justify-between border-t border-slate-100 px-5 py-3 text-[11px] font-bold text-slate-400 sm:px-8"><span>{words.toLocaleString()} words · {draft.blocks.length} blocks</span><span>{uploading ? 'Uploading attachment…' : saveState === 'offline' ? 'Local draft protected' : 'Ctrl/Cmd + K commands · Ctrl/Cmd + S save'}</span></div></div>
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-24"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Document outline</p><div className="mt-3 space-y-1">{headings.map((heading) => <button key={heading.id} onClick={() => window.document.getElementById(`vault-block-${heading.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })} className={`block w-full truncate rounded-lg px-2 py-2 text-left text-xs font-bold text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 ${heading.level === 3 ? 'pl-5' : heading.level === 2 ? 'pl-3' : ''}`}>{stripHtml(heading.content || '')}</button>)}{!headings.length && <p className="mt-3 text-xs leading-5 text-slate-400">Headings create a navigable outline automatically.</p>}</div></aside></div></div>
      {slash && <SlashMenu commands={filteredCommands} onSelect={applyCommand} onClose={() => setSlash(null)} />}{showPalette && <CommandPalette commands={slashCommands} onSelect={applyCommand} onClose={() => setShowPalette(false)} onNewDocument={() => { setBlocks([newBlock('paragraph')], true); setShowPalette(false); }} />}{showHistory && <HistoryPanel documentId={documentId} versions={versions} canRestore={canManage} onClose={() => setShowHistory(false)} onRestore={async (version) => { if (!documentId) return; const result = await db.vault.restoreDocumentVersion(documentId, version); const refreshed = await db.vault.document(documentId); setDraft(initialDraft(refreshed)); onSaved(refreshed); setSaveState('saved'); setSaveMessage(`Restored version ${version} as v${result.data.version}`); setShowHistory(false); }} />}</div>;
};
...
const EmbedBlock = ({ block, shell, controls, canEdit, onChange, onFocus }: any) => <div id={`vault-block-${block.id}`} className={`${shell} rounded-xl border border-slate-200 bg-slate-50 p-4`}>{controls}<div className="flex items-center gap-2 text-emerald-700"><Link2 className="h-4 w-4" /><span className="text-xs font-black uppercase tracking-wider">Linked resource</span></div>{canEdit ? <><input value={block.content || ''} onFocus={onFocus} onChange={(event) => onChange({ content: event.target.value })} placeholder="Link title — project, meeting, document, GitHub…" className="mt-3 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" /><input value={block.url || ''} onChange={(event) => onChange({ url: event.target.value })} placeholder="https://… or Vault reference" className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" /></> : <a href={block.url || '#'} target="_blank" rel="noreferrer" className="mt-3 block text-sm font-bold text-emerald-700 hover:underline">{stripHtml(block.content || block.url || 'Linked resource')}</a>}</div>;

const EditorToolbar = ({ onFormat, onInsert, onPalette, onUndo, onRedo, canUndo, canRedo, onUpload }: any) => <div className="flex gap-1 overflow-x-auto border-b border-slate-100 px-4 py-2.5 sm:px-8"><ToolbarButton label="Bold" onClick={() => onFormat('bold')}><Bold className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Italic" onClick={() => onFormat('italic')}><Italic className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Underline" onClick={() => onFormat('underline')}><Underline className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Strikethrough" onClick={() => onFormat('strikeThrough')}><Strikethrough className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Highlight" onClick={() => onFormat('hiliteColor', '#fef08a')}><span className="text-[10px] font-black">HL</span></ToolbarButton><span className="editor-toolbar-divider" /><ToolbarButton label="Heading 1" onClick={() => onInsert('heading', { level: 1 })}><Heading1 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Heading 2" onClick={() => onInsert('heading', { level: 2 })}><Heading2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Bullets" onClick={() => onInsert('bulletList')}><List className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Numbered list" onClick={() => onInsert('numberedList')}><ListOrdered className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Checklist" onClick={() => onInsert('checklist')}><Check className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Quote" onClick={() => onInsert('quote')}><Quote className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Code block" onClick={() => onInsert('code')}><Code2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Table" onClick={() => onInsert('table')}><Table2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Attachment" onClick={onUpload}><Paperclip className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Link / GitHub / project" onClick={() => onInsert('embed')}><Link2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Divider" onClick={() => onInsert('divider')}><Minus className="h-4 w-4" /></ToolbarButton><span className="editor-toolbar-divider" /><ToolbarButton label="Undo" disabled={!canUndo} onClick={onUndo}><Undo2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Redo" disabled={!canRedo} onClick={onRedo}><Redo2 className="h-4 w-4" /></ToolbarButton><ToolbarButton label="Command palette" onClick={onPalette}><Command className="h-4 w-4" /></ToolbarButton></div>;
