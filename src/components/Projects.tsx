        ...
        <AnimatePresence mode="wait">
          {!selectedProject ? (
                ...
                {projects.map((project, index) => (
                  ...
                  >
                    <div className="relative flex h-44 items-center justify-center overflow-hidden border-b border-[#b8ff3d]/15 bg-[#07150b]">
                      ...
                      <span className="absolute left-5 top-5 rounded-full border border-[#b8ff3d]/25 bg-[#b8ff3d]/8 px-3 py-1.5 text-[0.64rem] font-black uppercase tracking-[0.13em] text-[#b8ff3d]">{project.category}</span>
                    ...
                    <div className="flex flex-grow flex-col p-6 sm:p-7">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <ArrowRight className="h-5 w-5 shrink-0 text-[#94a992] transition-all group-hover:translate-x-1 group-hover:text-[#b8ff3d]" />
                      </div>
                      <p className="mt-3 flex-grow text-sm leading-7 text-[#a9bda3]">{project.description}</p>
                      <div className="mt-6 flex flex-wrap gap-2">
                        {project.technology.slice(0, 3).map((tech, i) => <span key={i} className="rounded-md border border-[#b8ff3d]/15 bg-[#b8ff3d]/5 px-2.5 py-1 text-[0.64rem] font-bold uppercase tracking-wide text-[#a9bf9f]">{tech}</span>)}
                      </div>
                      <div className="mt-6 flex items-center justify-between border-t border-[#b8ff3d]/12 pt-4">
                        <span className="text-[0.64rem] font-black uppercase tracking-[0.16em] text-[#94a992]">{project.status}</span>
                        <span className="text-[0.65rem] font-black uppercase tracking-[0.15em] text-[#b8ff3d]">Open case →</span>
                      </div>
                    </div>
                  </motion.button>
                ))}
            ...
            <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="mx-auto max-w-6xl">
              <button type="button" onClick={() => setSelectedProject(null)} className="mb-10 flex items-center gap-2 text-[0.68rem] font-black uppercase tracking-[0.16em] text-[#a9bda3] transition-colors hover:text-[#b8ff3d]"><ArrowLeft className="h-4 w-4" /> Back to lab</button>
              <div className="grid gap-12 lg:grid-cols-[1fr_320px]">
                <div>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-4 text-[0.68rem] font-bold uppercase tracking-[0.13em] text-[#a9bda3]">
                    <span className="text-[#b8ff3d]">{selectedProject.status}</span><span className="h-1 w-1 rounded-full bg-[#94a992]" /><span className="brand-mono">Progress: {selectedProject.progress}%</span>
                  </div>
                  ...
                  <div className="brand-card mt-12 p-7 sm:p-10">
                    ...
                    <div className="grid gap-8 sm:grid-cols-2">
                      <div><p className="brand-number mb-2">01 / PROBLEM</p><p className="text-sm leading-7 text-[#a9bda3]">{selectedProject.problem}</p></div>
                      <div><p className="brand-number mb-2">02 / SOLUTION</p><p className="text-sm leading-7 text-[#a9bda3]">{selectedProject.solution}</p></div>
                    </div>
                  </div>
                ...
                <aside className="brand-card h-fit p-6 sm:p-7">
                  ...
                  <div className="mt-10 border-t border-[#b8ff3d]/15 pt-7"><p className="text-xs leading-6 text-[#a9bda3]">Want to help move this project forward? Join the society and contribute your perspective.</p><button type="button" className="brand-button mt-5 w-full">Join this project</button></div>
                </aside>
              </div>
            </motion.div>
          )}
