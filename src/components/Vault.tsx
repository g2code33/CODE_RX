...
import { Archive, ArrowLeft, BookOpen, ChevronRight, FilePlus2, FileText, FolderKanban, History, Home, LockKeyhole, Menu, Search, Upload, X } from 'lucide-react';
...
/**
 * The Vault shelf, grouped by whose papers these are.
 *
 * A document published to a client carries that client and their project (the
 * link already exists in `client_documents.vault_document_id`), so the shelf is
 * grouped by client first and project second. Documents that never went to a
 * client sit under one honest heading — nothing is guessed.
 */
/** Exported so the grouping rule itself is testable, not just its rendering. */
export const groupDocumentsByClient = (documents: any[]) => {
  const groups: Array<{ key: string; client: string; project: string; documents: any[] }> = [];
  documents.forEach((document) => {
    const client = document.client_name || '';
    const project = document.client_project_name || '';
    const key = client ? `${client}::${project}` : '__internal__';
    let group = groups.find((entry) => entry.key === key);
    if (!group) {
      group = { key, client, project, documents: [] };
      groups.push(group);
    }
...
const SectionList = ({ section, documents, projects, loading, canCreate, canManage, archived, archivedProjects, onToggleArchived, onToggleArchivedProjects, onNew, onUpload, onOpen, onUnarchive, onArchiveProject }: any) => <div className="vault-page"><div className="vault-page-heading vault-page-heading--row"><div><p className="vault-kicker">{section?.slug}</p><h1>{archived ? `${section?.title} Archive` : section?.title}</h1><p>{archived ? 'Archived documents are preserved and can be restored by a section manager.' : section?.description}</p></div><div className="flex flex-wrap gap-2">{canManage && <button onClick={onToggleArchived} className="vault-appbar-button">{archived ? 'Active Documents' : 'Archived Documents'}</button>}{canCreate && !archived && <button onClick={onUpload} className="vault-appbar-button"><Upload className="h-4 w-4" />Upload Document</button>}{canCreate && !archived && <button onClick={onNew} className="vault-new-button"><FilePlus2 className="h-4 w-4" />New Document</button>}</div></div>{loading ? <div className="grid min-h-60 place-items-center"><p className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700">Loading documents…</p></div> : <><div className="vault-table-wrap"><table className="vault-document-table"><thead><tr><th>Name</th><th>Updated</th><th>Author</th><th>Status</th>{archived && <th className="text-right">Restore</th>}</tr></thead><tbody>{groupDocumentsByClient(documents).map((group: any) => <Fragment key={group.key}><tr className="vault-client-group-row"><td colSpan={archived ? 5 : 4}><span className="vault-client-group__client">{group.client || 'Internal — not published to a client'}</span><span className="vault-client-group__project">{group.client ? (group.project || 'No project named') : 'Vault documents that stay inside Code Rx'}</span><span className="vault-client-group__count">{group.documents.length}</span></td></tr>{group.documents.map((document: any) => <tr key={document.id} onClick={() => !archived && onOpen(document.id)}><td><span className="flex items-center gap-3"><FileText className="h-4 w-4 text-emerald-600" /><span className="flex min-w-0 flex-col"><strong>{document.title}</strong>{document.document_code && <small className="mt-0.5 text-[10px] font-black uppercase tracking-wider text-slate-400">{document.document_code}</small>}</span></span></td><td>{dateLabel(document.updated_at)}</td><td>{document.updated_by_name || document.created_by_name || 'Code Rx member'}</td><td><span className="vault-status-pill">{archived ? document.archived_from_status || 'draft' : document.status || 'draft'}</span></td>{archived && <td className="text-right"><button onClick={(event) => { event.stopPropagation(); onUnarchive(document.id); }} className="mini-button text-emerald-700">Unarchive</button></td>}</tr>)}</Fragment>)}{!documents.length && <tr><td colSpan={archived ? 5 : 4}><EmptyState label={archived ? 'No archived documents in this section.' : 'No documents in this section yet.'} /></td></tr>}</tbody></table></div>{section?.slug === 'projects' && !archived && <ProjectLinks projects={projects} canManage={canManage} archived={archivedProjects} onToggleArchived={onToggleArchivedProjects} onArchive={onArchiveProject} />}</>}</div>;
