...
export const SiteFlow = ({
  ...
  const page = (() => {
    switch (activeTab) {
      case 'home':
        return <><Hero content={content.home} copy={copy} media={content.media} onJoin={onJoin} /><ValueCards values={content.home.coreValues} copy={copy} /><EditableRegion elementKey="news.section" label="Latest news section" collection="news"><section id="news" className="brand-section brand-section--panel border-y border-[#16a34a]/20 py-24 sm:py-28"><PharmacyBackground layout="lab" /><div className="relative z-10 mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-10"><div className="mb-12 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><div className="brand-eyebrow mb-5"><EditableText elementKey="news.eyebrow" copyKey="news.eyebrow" label="News eyebrow">{getCopy(copy, 'news.eyebrow', 'Latest signal')}</EditableText></div><h2 className="brand-title text-4xl sm:text-5xl"><EditableText elementKey="news.title" copyKey="news.title" label="News heading">{getCopy(copy, 'news.title', 'What’s moving')}</EditableText><br /><span className="brand-gradient-text"><EditableText elementKey="news.title-accent" copyKey="news.titleAccent" label="News heading accent">{getCopy(copy, 'news.titleAccent', 'the network.')}</EditableText></span></h2></div><SectionLink id="news" /></div>{content.home.latestNews.length ? (
              <EditableRegion elementKey="news.grid" label="News card grid" collection="news" className="grid gap-4 md:grid-cols-3">{content.home.latestNews.map((news, index) => <EditableRegion key={news.id} elementKey={`news.card.${news.id}`} label={`${news.title} news card`} collection="news" className="brand-card brand-card-hover p-6 sm:p-7"><div className="flex items-center justify-between"><span className="brand-number">0{index + 1} / <EditableText elementKey={`news.card.${news.id}.category`} copyKey={`home.latestNews.${index}.category`} label={`${news.title} category`}>{news.category}</EditableText></span><span className="h-1.5 w-1.5 rounded-full bg-[#15803d] shadow-[0_0_10px_rgba(21,128,61,0.45)]" /></div><h3 className="mt-8 text-xl font-black leading-tight tracking-tight text-[#0f172a]"><EditableText elementKey={`news.card.${news.id}.title`} copyKey={`home.latestNews.${index}.title`} label={`${news.title} title`}>{news.title}</EditableText></h3><p className="mt-4 text-sm leading-7 text-[#475569]"><EditableText elementKey={`news.card.${news.id}.text`} copyKey={`home.latestNews.${index}.text`} label={`${news.title} text`}>{news.text}</EditableText></p><div className="mt-7 h-px w-12 bg-[#15803d]/60" /></EditableRegion>)}</EditableRegion>
            ) : (
              <SectionEmpty
                title="No announcements yet."
                note="Society announcements, events and research notes are published here."
              />
            )}</div></section></EditableRegion></>;
      case 'about':
