          ...
          <div className="grid gap-14 lg:grid-cols-[1.35fr_0.8fr_0.8fr_1fr]">
            <div>
              <a href="#home" className="inline-flex items-center gap-3 no-underline">
                ...
                <span><span className="block text-lg font-black tracking-[0.12em] text-[#f2f8ed]">CODE <span className="text-[#b8ff3d]">Rx</span></span><span className="mt-1 block text-[0.66rem] font-black uppercase tracking-[0.26em] text-[#a9bda3]">Society / Ghana</span></span>
              </a>
              <p className="mt-7 max-w-sm text-sm leading-7 text-[#a9bda3]">Equipping pharmacists with the technological skills to innovate, build, and lead in the digital healthcare era.</p>
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => <label className="block text-[0.66rem] font-black uppercase tracking-[0.16em] text-[#a9bda3]">{label}{children}</label>;
const PolicyContent = ({ children }: { children: React.ReactNode }) => <div className="space-y-5 text-sm leading-7 text-[#a9bda3]">{children}</div>;
const Modal = ({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) => <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" onClick={onClose}><div className="brand-card max-h-[90vh] w-full max-w-xl overflow-y-auto p-7 sm:p-9" onClick={(event) => event.stopPropagation()}><div className="mb-7 flex items-start justify-between gap-4"><h2 className="text-2xl font-black text-[#f2f8ed]">{title}</h2><button type="button" onClick={onClose} aria-label="Close dialog" className="text-[#a9bda3] transition-colors hover:text-[#b8ff3d]"><X className="h-5 w-5" /></button></div>{children}</div></div>;
