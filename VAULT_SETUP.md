...
- PHANTOM-controlled document sharing: separate global and per-member Share-button and Download-button controls. Links are random, read-only, non-expiring until revoked, and can independently allow or block shared-page download/print. Active links have a **Copy link** control in the Existing links list; token copies are encrypted at rest and only returned to an authorized owner. Historic links created before this upgrade can be safely replaced with a new copyable URL. They never expose protected attachments or sensitive/restricted documents.
